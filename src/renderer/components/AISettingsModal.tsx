...
export const AISettingsModal: React.FC<AISettingsModalProps> = ({
  ...
  const loadExistingCredentials = async () => {
    try {
      // First, check if there's a saved provider preference
      const savedProvider = localStorage.getItem("ai-selected-provider");

      if (savedProvider) {
        // Try to load credentials for the saved provider
        const credentialsStr =
          await window.electronAPI.ai.getCredentials(savedProvider);
        if (credentialsStr) {
        ...
        // If no credentials for saved provider, just set the provider
        setCredentials((prev) => ({
          ...prev,
          provider: savedProvider as AICredentials["provider"],
          model:
            defaultModels[savedProvider as keyof typeof defaultModels]?.[0] ||
            prev.model,
        }));
      } else {
        // No saved preference, check all providers for saved credentials
        const providers = ["openai", "anthropic", "azure", "bedrock", "ollama"];
        ...
        for (const provider of providers) {
          ...
          if (credentialsStr) {
            ...
            // Save this as the selected provider
      ...
      // No credentials found, keep defaults
      setIsLoading(false);
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save the selected provider to localStorage
      localStorage.setItem("ai-selected-provider", credentials.provider);
        ...
        return (
          <div className="form-group">
            ...
            <input
              type="url"
              id="baseUrl"
              value={credentials.baseUrl || "http://127.0.0.1:11434/api"}
              onChange={(e) =>
  ...
  return (
    <div
      ...
      onClick={onClose}
      data-testid="ai-settings-modal-overlay"
    >
      <div
        ...
        >
          <div className="connection-modal-body">
            ...
            <div className="form-group">
              ...
              >
                <option value="openai" data-testid="ai-provider-option">
                  OpenAI
                </option>
                <option value="anthropic" data-testid="ai-provider-option">
                  Anthropic
                </option>
                <option value="azure" data-testid="ai-provider-option">
                  Azure OpenAI
                </option>
                <option value="bedrock" data-testid="ai-provider-option">
                  Amazon Bedrock
                </option>
                <option value="ollama" data-testid="ai-provider-option">
                  Ollama (Local)
                </option>
              </select>
            </div>
            ...
            <div className="info-message">
              <strong>Note:</strong> When saving, credentials will be validated
              by generating a single token using the selected model to ensure
              they work correctly.
            </div>
          </div>
