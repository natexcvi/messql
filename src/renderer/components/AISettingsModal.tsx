...
const defaultModels = {
  ...
  bedrock: ['anthropic.claude-3-5-sonnet-20241022-v2:0', 'anthropic.claude-3-5-haiku-20241022-v1:0', 'anthropic.claude-3-opus-20240229-v1:0'],
  ollama: ['llama3.2', 'llama3.1', 'codellama', 'mistral'],
};

export const AISettingsModal: React.FC<AISettingsModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const loadExistingCredentials = async () => {
    try {
      const credentialsStr = await window.electronAPI.ai.getCredentials(credentials.provider);
      if (credentialsStr) {
  ...
  const handleProviderChange = useCallback(async (provider: AICredentials['provider']) => {
    setCredentials(prev => ({
      provider,
    ...
    try {
      ...
      if (credentialsStr) {
        const existingCredentials = JSON.parse(credentialsStr);
        setCredentials(prev => ({ ...prev, ...existingCredentials, provider }));
      }
    } catch (error) {
  ...
  const validateCredentials = async () => {
    ...
    try {
      const isValid = await window.electronAPI.ai.validateCredentials(credentials);
      if (!isValid) {
          ...
          <>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setCredentials(prev => ({ ...prev, resourceName: e.target.value }))}
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
          <div className="connection-modal-body">
            ...
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => handleProviderChange(e.target.value as AICredentials['provider'])}
          ...
          <div className="connection-modal-footer">
            ...
            <button 
              ...
              disabled={isLoading || isValidating || !credentials.apiKey && credentials.provider !== 'ollama'}
            >
              {isValidating ? (
