...
export class TestDatabase {
  ...
  static readonly config: TestDatabaseConfig = {
    ...
    port: process.env.TEST_DB_PORT || TestDatabase.containerPort,
    database: process.env.TEST_DB_NAME || "world-db",
    username: process.env.TEST_DB_USER || "world",
    password: process.env.TEST_DB_PASS || "world123",
  };
  ...
  static async start(): Promise<void> {
    ...
    try {
      execSync(`docker ps -a | grep ${this.containerName}`, {
        stdio: "ignore",
      });
